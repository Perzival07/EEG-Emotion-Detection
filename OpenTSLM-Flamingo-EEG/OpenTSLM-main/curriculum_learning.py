...
CURRICULUM_STAGES = [
    "stage6_brain_eeg",  # Brain EEG emotion classification
]
...
class CurriculumTrainer:
    ...
    def __init__(
        ...
        self.brain_eeg_dir = brain_eeg_dir
    ...
    def _train_stage(
        ...
        if sampler is not None:
            if self.world_size > 1:
                ...
                train_loader = self._merge_data_loaders(
                    [
                        dataset_class(
                            "train", EOS_TOKEN=self._get_model().get_eos_token(), **(dataset_kwargs or {})
                        )
                    ],
                    shuffle=True,
                    batch_size=batch_size,
            ...
            train_loader = self._merge_data_loaders(
                [dataset_class("train", EOS_TOKEN=self._get_model().get_eos_token(), **(dataset_kwargs or {}))],
                shuffle=True,
                batch_size=batch_size,
                patch_size=PATCH_SIZE,
        ...
        test_loader = self._merge_data_loaders(
            [dataset_class("test", EOS_TOKEN=self._get_model().get_eos_token(), **(dataset_kwargs or {}))],
            shuffle=False,
            batch_size=1,
            patch_size=PATCH_SIZE,
            distribute_data=self.world_size > 1,
    ...
    def stage6_brain_eeg(
        self, batch_size: int = None, eval_only: bool = False
    ) -> Dict[str, Any]:
        """Stage 6: Brain EEG Emotion Classification.

        Configuration:
        - Epochs: 30
        - OpenTSLMSP: encoder_lr=2e-4, projector_lr=1e-4
        - OpenTSLMFlamingo: base_lr=2e-4
        - Metric: Accuracy (emotion classification)
        """
        return self._train_stage(
            stage_name="stage6_brain_eeg",
            dataset_class=BrainEEGQADataset,
            num_epochs=30,
            lr_encoder=2e-4,
            lr_projector=1e-4,
            lr_base=2e-4,
            ...
            batch_size=batch_size,
            eval_only=eval_only,
            dataset_kwargs={"data_dir": self.brain_eeg_dir} if self.brain_eeg_dir else None,
        )
    ...
    def run_curriculum(
        ...
        for stage in incomplete_stages:
            ...
            if stage == "stage6_brain_eeg":
                stage_results = self.stage6_brain_eeg(
    ...
    def _enable_lora_if_needed(self, stage_name: str):
        ...
        stages_with_lora = ["stage3_cot", "stage4_sleep_cot", "stage5_ecg_cot", "stage6_brain_eeg"]
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to the Brain EEG dataset directory",
    )
    ...
    trainer = CurriculumTrainer(
        ...
        llm_id=args.llm_id,
        brain_eeg_dir=args.brain_eeg_dir,
    )
